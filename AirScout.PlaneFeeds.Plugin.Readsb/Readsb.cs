...
namespace AirScout.PlaneFeeds.Plugin.Readsb
{

    public class ReadsbServerSettings
    {
        ...
        [DefaultValue("http://localhost/tar1090/data/aircraft.json")]
        public string URL { get; set; }

        public ReadsbServerSettings()
        {
            Default();
            Load(true);
        }
    ...
    public class ReadsbPlugin : IPlaneFeedPlugin
    {
        private ReadsbServerSettings Settings = new ReadsbServerSettings();
        ...
        public string Info
        {
            get
            {
                return "Web feed from Readsb\n\n" +
                       "Use this feed to load an aircraft.json file from\n" +
                       "in-build webserver of Readsb.exe.\n" +
                       "Please see details here: https://github.com/wiedehopf/readsb/blob/dev/README-json.md" +
            ...
            try
            {
                ...
                foreach (VirtualRadar.Interface.WebSite.AircraftJson ac in AircraftList.Aircraft)
                {
                    try
                    {
                        ...
                        try
                        {
                            plane.Alt = (ac.AltitudeBaro == null) ? int.MinValue : System.Convert.ToInt32(ac.AltitudeBaro);
                        }
                        catch (Exception ex)
                        {
                            plane.Alt = 0;
