...
namespace AirScout.PlaneFeeds.Plugin.ADSBOne
{

    public class ADSBOneServerSettings
    {
        ...
        [DefaultValue("https://api.adsb.one/v2/point/%MYLAT%/%MYLON%/%MAXDISTKM%")]
        public string URL { get; set; }

        public ADSBOneServerSettings()
        {
            Default();
            Load(true);
        }
    ...
    public class ADSBOnePlugin : IPlaneFeedPlugin
    {
        private ADSBOneServerSettings Settings = new ADSBOneServerSettings();
        ...
        public string Info
        {
            get
            {
                return "Web feed from ADS-B One\n\n" +
                       "For details see https://adsb.one/.\n\n" +
                       "The use of this data is solely for your own personal, noncommercial use.\n" +
                       "Please see the Terms of Use here: https://adsb.one/data-tou\n\n" +
                       ...
                       "The data format for this feed is ADSBExchange V2.0 as published here: https://www.adsbexchange.com/version-2-api-wip/\n" +
                       "It can also be used for other web feeds following this specification.";
            }
        }
        ...
        public PlaneFeedPluginPlaneInfoList GetPlanes(PlaneFeedPluginArgs args)
        {
            ...
            try
            {
                ...
                // get JSON data
                Console.WriteLine("Getting JSON file from : " + url);
            ...
            try
            {
                ...
                if (AircraftList.FileGenerated != null)
                {
                    filetime = new System.DateTime(1970, 1, 1, 0, 0, 0, 0);
                    filetime = filetime.AddMilliseconds((long)AircraftList.FileGenerated);
                }
                ...
                foreach (VirtualRadar.Interface.WebSite.AircraftJson ac in AircraftList.Aircraft)
                {
                    try
                    {
                        ...
                        // get callsign
                        plane.Call = (ac.Callsign == null) ? "" : ac.Callsign.Trim().ToUpper();
                        // get registration
                        ...
                        if (ac.SeenPosition != null)
                        {
                            plane.Time = filetime.AddSeconds(-(double)ac.SeenPosition);
                        }
                        else
                        {
                        ...
                        if ((ac.Model != null) && (ac.Model.Contains(" ")))
                        {
                            List<string> a = ac.Model.Split(' ').ToList();
                            plane.Manufacturer = a.ElementAt(0)[0] + a.ElementAt(0).Substring(1, a.ElementAt(0).Length - 1).ToLower();
                            a.RemoveAt(0);
                            plane.Model = String.Join(" ", a);
                        }
                        else
                        {
                            plane.Model = ac.Model;
                        }
                        if (ac.WakeTurbulenceCategory == "A1")
                            plane.Category = 1;
                        else if (ac.WakeTurbulenceCategory == "A2")
                            plane.Category = 1;
                        else if (ac.WakeTurbulenceCategory == "A3")
                            plane.Category = 2;
                        else if (ac.WakeTurbulenceCategory == "A4")
                            plane.Category = 2;
                        else if (ac.WakeTurbulenceCategory == "A5")
                            plane.Category = 3;
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("Error while deserializing JSON file: " + ex.ToString());
