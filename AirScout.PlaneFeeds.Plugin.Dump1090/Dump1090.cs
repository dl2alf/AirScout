...
namespace AirScout.PlaneFeeds.Plugin.Dump1090
{

    public class Dump1090ServerSettings
    {
        ...
        [DefaultValue("http://localhost:8080/data/aircraft.json")]
        public string URL { get; set; }

        public Dump1090ServerSettings()
        {
            Default();
            Load(true);
        }
    ...
    public class Dump1090Plugin : IPlaneFeedPlugin
    {
        private Dump1090ServerSettings Settings = new Dump1090ServerSettings();
        ...
        public string Info
        {
            get
            {
                return "Web feed from Dump1090\n\n" +
                       "Use this feed to load an aircraft.json file from\n" +
                       "in-build webserver of dump1090.exe.\n" +
        ...
        public PlaneFeedPluginPlaneInfoList GetPlanes(PlaneFeedPluginArgs args)
        {
            ...
            try
            {
                ...
                if (AircraftList.Now != null)
                {
                    ...
                    filetime = filetime.AddMilliseconds((long)AircraftList.Now * 1000);
                }
                ...
                foreach (VirtualRadar.Interface.WebSite.AircraftJson ac in AircraftList.Aircraft)
                {
                    try
                    {
                        ...
                        if (ac.WakeTurbulenceCategory == "A1")
                            plane.Category = 1;
                        if (ac.WakeTurbulenceCategory == "A2")
                            plane.Category = 1;
                        if (ac.WakeTurbulenceCategory == "A3")
                            plane.Category = 2;
                        if (ac.WakeTurbulenceCategory == "A4")
                            plane.Category = 2;
                        if (ac.WakeTurbulenceCategory == "A5")
                            plane.Category = 3;
